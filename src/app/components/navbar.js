...
export default function NavbarHero() {
    ...
    return (
        <div className="w-full min-h-screen bg-gradient-to-br from-[#1A3A6D] to-[#0d1f3a]">
            {/* Navbar */}
<nav
    className="w-full bg-gray-100/80 backdrop-blur-xl shadow-xl"
...
>
                ...
                <div className="max-w-7xl mx-auto px-6">
                    <div className="flex items-center justify-between h-20">
                        ...
                        <div className="flex items-center space-x-3 cursor-pointer relative overflow-hidden">
                            ...
                            />
                            <span className="text-2xl font-bold text-white animate-slideDown">
                                BrandName
                            </span>
                        ...
                        <div className="hidden lg:flex items-center space-x-8">
                            {navLinks.slice(0, 3).map((link) => (
                                ...
                                >
                                    <button className="text-white font-semibold px-4 py-2 rounded-lg hover:bg-white/10 transition-all flex items-center">
                                        {link.name}
                                        <svg
                ...
                <div className="hidden lg:block relative">
                    {/* Services */}
                    <div
                        className={`absolute left-0 w-full mt-2 py-8 px-6 bg-[#1A3A6D]/98 backdrop-blur-2xl shadow-2xl transition-all duration-300 ${activeMenu === "services"
                    ...
                    {/* Products */}
                    <div
                        className={`absolute left-0 w-full mt-2 py-8 px-6 bg-[#1A3A6D]/98 backdrop-blur-2xl shadow-2xl transition-all duration-300 ${activeMenu === "products"
                            ? "opacity-100 visible translate-y-0"
                            : "opacity-0 invisible -translate-y-5"
                            }`}
                    >
                    ...
                    {/* Company */}
                    <div
                        className={`absolute left-0 w-full mt-2 py-8 px-6 bg-[#1A3A6D]/98 backdrop-blur-2xl shadow-2xl transition-all duration-300 ${activeMenu === "company"
                            ? "opacity-100 visible translate-y-0"
                            : "opacity-0 invisible -translate-y-5"
                            }`}
                    >
                        <div className="max-w-7xl mx-auto grid grid-cols-2 gap-8">
                            ...
                            <div className="p-6 rounded-xl border border-[#4DA6FF]/30 bg-gradient-to-br from-[#4DA6FF]/10 to-[#4DA6FF]/5">
                                ...
                                <button className="px-6 py-2 rounded-lg font-semibold text-white transition-all bg-gradient-to-br from-[#4DA6FF] to-[#2d7fd9] shadow-lg hover:shadow-xl">View Open Positions</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-20 text-center">
                <h1 className="text-4xl sm:text-5xl font-bold text-white mb-6">Welcome to Our Platform</h1>
                <p className="text-lg sm:text-xl text-gray-300 mb-8">Hover over the navigation items to see the animated mega menus in action</p>
                <div className="flex flex-col sm:flex-row justify-center items-center sm:space-x-4 space-y-4 sm:space-y-0">
                    <HeroCard title="Fast Performance" desc="Lightning-quick load times" icon={performanceIcon} />
                    <HeroCard title="Secure" desc="Enterprise-grade security" icon={securityIcon} />
                    <HeroCard title="Reliable" desc="99.9% uptime guarantee" icon={analyticsIcon} />
                </div>
            </div>
        </div>
    );
...
// Hero Card Component
const HeroCard = ({ title, desc, icon }) => (
    <div className="p-8 rounded-xl bg-[#4DA6FF]/10 backdrop-blur-xl border border-[#4DA6FF]/30 w-full sm:w-auto">
...
// Dummy Icons (Replace with your SVGs)
